...
def process_video():
    ...
    处理上传的视频，实时返回处理后的帧
    """
    global processing_results, global_config
    input_path = request.args.get('video_path')
    ...
    def generate_frames():
        ...
        processing_results.update({
            ...
            'csv_path': os.path.join(app.config['RESULT_FOLDER'], 
                                   f"speeds_{os.path.splitext(os.path.basename(input_path))[0]}.csv")
        })
        ...
        while cap.isOpened():
            ...
            # 更新处理结果
            processing_results['vehicle_counts'].append(frame_vehicle_count)
            processing_results['wrong_way_count'] = wrong_way_count
            processing_results['vehicle_data'] = vehicle_data
            ...
            # 将帧转换为JPEG格式并编码为base64
            _, buffer = cv2.imencode('.jpg', frame)
            frame_base64 = base64.b64encode(buffer).decode('utf-8')
            
            # 使用SSE发送帧
            yield f"data: {json.dumps({'type': 'frame', 'frame': frame_base64})}\n\n"
        ...
        if writer:
            writer.release()
        processing_results['video_path'] = f"/results/processed_{os.path.splitext(os.path.basename(input_path))[0]}_h264.mp4"
        # 转换为H.264编码
        h264_output_path = os.path.join(app.config['RESULT_FOLDER'], 
            f"processed_{os.path.splitext(os.path.basename(input_path))[0]}_h264.mp4")
        os.system(f"ffmpeg -i {output_path} -c:v libx264 -c:a aac -strict experimental {h264_output_path}")
        
        # 保存CSV数据
        with open(processing_results['csv_path'], mode='w', newline='') as csv_file:
            fieldnames = ['track_id', 'max_speed']
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
            writer.writeheader()
            for track_id, data in processing_results['vehicle_data'].items():
                writer.writerow({
                    'track_id': track_id, 
                    'max_speed': data.get('max_speed', 0)
                })

        video_path=processing_results['video_path']
        # 发送处理完成信号
        yield f"data: {json.dumps({'type': 'complete', 'video_path': video_path})}\n\n"
...
def get_results():
    ...
    return jsonify({
        'video_path': processing_results['video_path'],
        'csv_path': f"/results/{os.path.basename(processing_results['csv_path'])}",
        'vehicle_counts': processing_results['vehicle_counts'],
        'wrong_way_count': processing_results['wrong_way_count']
    })
